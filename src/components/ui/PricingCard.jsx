  ...
  return (
    <div
      className={`relative rounded-[2rem] p-8 flex flex-col transition-all duration-300 ${
        plan.highlight
          ? 'bg-primary-500/5 border-2 border-primary-500/40 shadow-[0_0_40px_rgba(217,26,84,0.15)]'
          : 'sc-card hover:border-white/20'
      }`}
    >
      {plan.badge && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary-500 text-white text-xs font-semibold px-4 py-1 rounded-full">
          {plan.badge}
        </span>
      )}
